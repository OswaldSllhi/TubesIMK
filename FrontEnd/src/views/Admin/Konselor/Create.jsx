...
export default function KonselorCreate() {
  document.title = "Membuat Konselor";

  const navigate = useNavigate();
  const [name, setName] = useState("");
  const [image, setImage] = useState("");
  const [role, setRole] = useState(""); // Mengubah default value role menjadi string kosong ("")
  const [phone, setPhone] = useState("");
  const [errors, setErrors] = useState([]);

  const token = Cookies.get("token");

  const storeKonselor = async (e) => {
    e.preventDefault();
    const formData = new FormData();
    formData.append("image", image);
    formData.append("name", name);
    formData.append("role", role); // Menambahkan data peran ke formData
    formData.append("phone", phone);

    try {
      const response = await Api.post("/api/admin/konselors", formData, {
        headers: {
            ...
            <div className="col-md-12">
              ...
              <div className="card border-0 rounded shadow-sm border-top-success">
                <div className="card-body">
                  <h6>
                    <i className="fa fa-pencil"></i> Tambah Konselor
                  </h6>
                  <hr />
                  <form onSubmit={storeKonselor}>
                    <div className="mb-3">
